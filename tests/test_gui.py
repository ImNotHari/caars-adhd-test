...
def test_gui_initialization(test_app):
    assert test_app.title() == "CAARS Adult ADHD Assessment & Scoring System (Offline)"
    assert test_app.current_view is not None

def test_gui_patient_creation_and_navigation(test_app):
    # Create patient via DB
    p_id = test_app.db.create_patient(
        mrn="GUI-TEST-01",
        first_name="Sam",
        last_name="Rivers",
        dob="1990-03-20",
    ...
    test_app.show_dashboard()
    test_app.update_idletasks()

    # Navigate to assessment view
    test_app.start_assessment(patient_id=p_id, form_type="self")
    test_app.update_idletasks()

    # Verify assessment view loaded
    from caars.gui.views.assessment_view import AssessmentView
    assert isinstance(test_app.current_view, AssessmentView)
    assert test_app.current_view.patient_id == p_id
    assert test_app.current_view.total_questions == 66

def test_gui_chart_rendering(test_app):
    from caars.gui.components.chart_canvas import TScoreChart

    chart = TScoreChart(test_app)
    sample_scores = [
